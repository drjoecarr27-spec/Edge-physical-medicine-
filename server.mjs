...
const IMAGE_SOURCE='https://d2jqrm6oza8nb6.cloudfront.net/datasets/7e76395d-d790-4563-8c7e-00899a32ca34.webp?_jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJrZXlIYXNoIjoiMjk1MjJmZjAzMDI2YWRiYyIsImJ1Y2tldCI6InJ1bndheS1kYXRhc2V0cyIsInN0YWdlIjoicHJvZCIsImV4cCI6MTc4OTY2ODA4MH0.DaKb8D1qXTP4P1-C_xiSVOkN7YW4qexNLFhKpfnoI0A';
...
const injection='<link rel="stylesheet" href="/_edge/theme.css"><script defer src="/_edge/enhance.js"></script>';
...
async function fetchSource(url,headers={}){
 ...
 const r=await fetch(u,{signal:AbortSignal.timeout(25000),redirect:'manual',headers:{'user-agent':'EdgeWebsiteVisualCopy/1.0',...headers}});
...
async function makeSnapshot(){
 await mkdir(ROOT,{recursive:true});let image;
 for(const u of [LIVE+'/_edge/corolla.webp',IMAGE_SOURCE]){try{const r=await fetch(u,{signal:AbortSignal.timeout(25000)});if(r.ok&&(r.headers.get('content-type')||'').startsWith('image/')){image=Buffer.from(await r.arrayBuffer());if(image.length>4000)break;}}catch{}}
 if(!image||image.length<4000)throw new Error('Corolla image could not be bundled');await writeFile(path.join(ROOT,'corolla.webp'),image);
 ...
 console.log('EDGE_SNAPSHOT',JSON.stringify(audit));
}

async function start(){
 const audit=JSON.parse(await readFile(path.join(ROOT,'audit.json'),'utf8'));
 const image=await readFile(path.join(ROOT,'corolla.webp'));
 const theme=await readFile('theme.css');const enhanced=await readFile('enhance.js');
 ...
 server.listen(Number(process.env.PORT)||8080,'0.0.0.0',()=>console.log('EDGE_READY',audit.pages.length,'pages; original English text preserved; Corolla bundled'));
}
