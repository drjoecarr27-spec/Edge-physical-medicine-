(() => {
 'use strict';
 const VISUAL='clinic-daylight-v3';
 ...
 const freshURL=value=>{try{const u=new URL(value,location.href);if(u.origin!==location.origin)return value;u.searchParams.set('edge_visual',VISUAL);return u.pathname+u.search+u.hash;}catch{return value;}};
 let activeObserver;
 const init=()=>{
  ...
  if(!document.getElementById('edge-fresh-css')){const css=document.querySelector('link[rel="stylesheet"][href*="/_next/"]');const theme=document.querySelector('link[rel="stylesheet"][href*="/_edge/theme.css"]');if(css&&theme){const fresh=document.createElement('link');fresh.rel='stylesheet';fresh.id='edge-fresh-css';fresh.href=freshURL(css.getAttribute('href'));theme.before(fresh);}}
  ...
  const patchImage=img=>{if(!(img instanceof HTMLImageElement))return;const src=img.getAttribute('src')||'';const alt=altFor(src);if(alt&&img.getAttribute('alt'))img.alt=alt;
   if(/\/edge\/chapters\//.test(src)){const next=freshURL(src);if(next!==src)img.setAttribute('src',next);const set=img.getAttribute('srcset');if(set){const fresh=set.split(',').map(v=>{const [u,...rest]=v.trim().split(/\s+/);return [freshURL(u),...rest].join(' ');}).join(', ');if(fresh!==set)img.setAttribute('srcset',fresh);}}
   if(/chapters\/01\/ch01-plate/.test(src)){img.dataset.edgePhoto='';img.setAttribute('fetchpriority','high');const section=img.closest('section');if(section)section.dataset.edgeFirst='';}};
  ...
  activeObserver.observe(root,{childList:true,subtree:true,attributes:true,attributeFilter:['style','fill','stroke','stop-color','src','srcset']});
  document.documentElement.dataset.edgeVisual=VISUAL;window.edgeVisualCheck={version:VISUAL,initialTextUnchanged:before===root.textContent};
